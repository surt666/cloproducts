...
(defn opret-leverings-aftale [abon-id betaler forbruger produkt-id prov-system prov-string logistic-string leverings-periode]
  (if (not (nil? prov-system))
    (provisioner prov-system prov-string (now)))
  (if (not (nil? logistic-string))
    (bestil-fra-lager logistic-string))
  (struct leverings-aftale abon-id produkt-id leverings-periode forbruger betaler))
      ...
      (for [p plist]
        (let [prod (find-produkt p)]
          (let [prov-system (:prov_system (meta prod))
            ...
            (opret-leverings-aftale abon-id betaler forbruger p prov-system prov-string logistic-string leverings-periode)))))))
